...
from .conftest import SAMPLE_RATE, WriteManifest, WriteRender, note_record, ramp
...
def test_every_note_of_a_manifest_is_cut_into_a_sample_of_its_own(
    ...
    result = trim_note_stream(wav_path, manifest_path, output_directory, TrimConfig())
...
def test_a_sample_holds_the_frames_its_note_sounds_over(
    ...
    trim_note_stream(wav_path, manifest_path, output_directory, TrimConfig())
...
def test_a_run_reports_the_samples_the_stream_ran_out_for(
    ...
    result = trim_note_stream(wav_path, manifest_path, tmp_path / "samples", config)
...
def test_a_run_whose_samples_all_fit_reports_none_as_clamped(
    ...
    result = trim_note_stream(wav_path, manifest_path, tmp_path / "samples", TrimConfig())
...
def test_the_number_of_decimals_a_run_keeps_reaches_the_sample_names(
    ...
    trim_note_stream(wav_path, manifest_path, output_directory, TrimConfig(cc_decimals=1))
...
def test_a_run_asked_to_overwrite_replaces_an_earlier_sample(
    ...
    trim_note_stream(wav_path, manifest_path, output_directory, TrimConfig(overwrite=True))
...
def test_a_manifest_that_was_never_written_is_reported(
    ...
    with pytest.raises(ManifestError, match="cannot read manifest"):
        trim_note_stream(wav_path, tmp_path / "absent.notes.json", tmp_path / "samples", TrimConfig())
...
def test_a_render_that_was_never_written_is_reported(
    ...
    with pytest.raises(AudioError, match="cannot read audio"):
        trim_note_stream(tmp_path / "absent.wav", manifest_path, tmp_path / "samples", TrimConfig())
